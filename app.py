...
def index():
    ...
    if conn is None:
        ...
        conn.close()
        db = True
    return render_template("index.html", oid_list=oid_list, db=db)
...
def show_traps():
    ...
    cursor.close()
    conn.close()
    return render_template("trap.html", traps=traps, start_date=start_date, end_date=end_date)

@app.route("/trap/<int:trap_id>", methods=["GET"])
...
def get_data(version, community=None, user=None, authkey=None, privkey=None, auth_protocol="NONE", priv_protocol="NONE"):
    if version in ['v1', 'v2c']:
...
def snmp_get(ip, community, oid, version, user=None, authkey=None, privkey=None, auth_protocol="NONE", priv_protocol="NONE"):
    result = []
    data = get_data(version, community, user, authkey, privkey, auth_protocol, priv_protocol)
    if not data:
